# Socrates's Profile

`#+AUTHOR:` **suxiaogang223** &nbsp; | &nbsp; `#+EMAIL:` **suxiaogang223@icloud.com**  

### `*` 👋 (message "Hello, World!")
> Data Engineer & Open Source Enthusiast. Currently brewing high-performance magic at **Apache Doris**.

### `**` 🚀 Buffers
- **[[Resume](https://github.com/suxiaogang223/resume)]** :: `(find-file "resume.org")` — *Grab my latest bits*
- **[[Blog](https://suxiaogang223.github.io)]**   :: `(browse-url "https://suxiaogang223.github.io")` — *Random thoughts & tech rants*
- **[[Emacs](https://github.com/suxiaogang223/kanso-emacs)]**  :: `(find-file "init.el")` — *My personal Emacs configuration, where "Kanso" (簡素) stands for simplicity and elimination of clutter.*
- **[[Doris](https://github.com/apache/doris)]**  :: `(visit-project "apache/doris")` — *Crafting the future of real-time analytics*

### `**` 🛠 (setq tech-stack)
- **Core** :: Apache Doris, Spark, Hive, Iceberg, Paimon
- **Lang** :: Rust (blazing fast!), C++, Java, Python, `(emacs-lisp-mode)`
- **Tools** :: VSCode & Emacs fans, deep in vibe coding 🎧

### `**` 🎯 Hacking Now: Current Focus `[0/4]`
- [ ] Contributing to Apache Doris development
- [ ] Build high-performance Data Lake engine with Rust 🦀
- [ ] Make my Emacs toy `mutype` shine ✨
- [ ] Finally bring `pymacs` to life (Python ❤️ Emacs)

### `**` 🕒 Recent Logs `(*Messages*)`
<!--START_SECTION:recent_activity-->
- 🔀 PR Merged: [branch-4.1: fix(outfile) handle delete_existing_files before parallel export ...](https://github.com/apache/doris/pull/61726) in `apache/doris` (2026-03-26)
- 🔀 PR Merged: [branch-4.1: fix(hive) Fix Hive DATE timezone shift in external readers #61330](https://github.com/apache/doris/pull/61722) in `apache/doris` (2026-03-26)
- 🔀 PR Merged: [branch-4.1: fix(catalog) avoid external catalog refresh deadlock #61202](https://github.com/apache/doris/pull/61721) in `apache/doris` (2026-03-26)
- 🔀 PR Merged: [fix(iceberg) Fix execute action validation gaps](https://github.com/apache/doris/pull/61381) in `apache/doris` (2026-03-25)
- 🔀 PR Merged: [fix(outfile) handle delete_existing_files before parallel export](https://github.com/apache/doris/pull/61223) in `apache/doris` (2026-03-23)
- 🔀 PR Merged: [Refactor(Multi Catalog) Unify external meta cache framework](https://github.com/apache/doris/pull/60937) in `apache/doris` (2026-03-21)
- 🆕 Repo Created: [tetris-60](https://github.com/suxiaogang223/tetris-60) (2026-03-20)
- 🔀 PR Merged: [fix(hive) Fix Hive DATE timezone shift in external readers](https://github.com/apache/doris/pull/61330) in `apache/doris` (2026-03-19)
- 🐛 Issue Opened: [Support querying system tables in paimon-cpp](https://github.com/alibaba/paimon-cpp/issues/141) in `alibaba/paimon-cpp` (2026-02-12)
- 🐛 Issue Opened: [RefactorTracking Unified external table metadata cache framework](https://github.com/apache/doris/issues/60686) in `apache/doris` (2026-02-11)
<!--END_SECTION:recent_activity-->

---
` ⏻ ` **[[README.org](https://github.com/suxiaogang223)]** ` ⎸ Ln 42:0 ⎸ All ⎸ ` **(Org Mode) ** ` ⚡ `
